...
exports.restoreManga = exports.deleteChapterPermanently = exports.getArchivedMangas = exports.unlockManga = exports.lockMangaForReview = exports.rejectProposal = exports.approveProposal = exports.getProposalForReview = exports.getPendingProposals = exports.deleteManga = exports.rejectChapter = exports.rejectManga = exports.approveChapter = exports.approveManga = exports.getMangaForReview = exports.getPendingEdits = exports.getNewSubmissions = void 0;
...
exports.getNewSubmissions = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const [submissions, total] = await db_1.default.$transaction([
        ...
        db_1.default.manga.count({ where: { status: 'PENDING' } })
    ]);
...
exports.getPendingEdits = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const [edits, total] = await db_1.default.$transaction([
        db_1.default.manga.findMany({
...
exports.getMangaForReview = (0, express_async_handler_1.default)(async (req, res) => {
...
exports.approveManga = (0, express_async_handler_1.default)(async (req, res) => {
    const { mangaId } = req.params;
    // Opcional: Chequeo rápido para ver si hay algo que aprobar.
    const mangaNeedsReview = await db_1.default.manga.findFirst({
        where: {
            id: mangaId,
            OR: [
                { status: 'PENDING' },
...
exports.approveChapter = (0, express_async_handler_1.default)(async (req, res) => {
    const { chapterId } = req.params;
    const chapterToApprove = await db_1.default.chapter.findUnique({
        where: { id: chapterId },
        select: {
            mangaId: true,
            chapterNumber: true,
...
exports.rejectManga = (0, express_async_handler_1.default)(async (req, res) => {
    const { mangaId } = req.params;
    ...
    const pendingChapters = await db_1.default.chapter.findMany({
        where: {
            mangaId: mangaId,
            status: 'PENDING'
        },
    ...
    if (manga.status === 'APPROVED') {
        ...
        const rejectChaptersPromise = db_1.default.chapter.updateMany({
            where: {
                mangaId: mangaId,
                status: 'PENDING',
            },
        ...
        const unlockPromise = db_1.default.manga.update({
            where: { id: mangaId },
            data: { lockedById: null, lockedAt: null }
        });
        ...
        // Se rechaza todo.
        const rejectMangaPromise = db_1.default.manga.update({
            where: { id: mangaId },
            data: {
                status: 'REJECTED',
        ...
        await db_1.default.$transaction([rejectMangaPromise, rejectChaptersPromise]);
...
exports.rejectChapter = (0, express_async_handler_1.default)(async (req, res) => {
    const { chapterId } = req.params;
    const { reason } = req.body; // Leemos el motivo del body
    // 1. Rechazamos el capítulo y guardamos el resultado
    const rejectedChapter = await db_1.default.chapter.update({
        where: { id: chapterId },
        data: {
            status: 'REJECTED',
            rejectionReason: reason
...
exports.deleteManga = (0, express_async_handler_1.default)(async (req, res) => {
    const { mangaId } = req.params;
...
exports.getPendingProposals = (0, express_async_handler_1.default)(async (req, res) => {
    const pendingProposals = await db_1.default.editProposal.findMany({
        where: { status: 'PENDING' },
        include: {
...
exports.getProposalForReview = (0, express_async_handler_1.default)(async (req, res) => {
    const { proposalId } = req.params;
    const proposal = await db_1.default.editProposal.findUnique({
        where: { id: proposalId },
        include: {
...
exports.approveProposal = (0, express_async_handler_1.default)(async (req, res) => {
    const { proposalId } = req.params;
    ...
    const dataToUpdate = {};
    if (proposal.title)
        dataToUpdate.title = proposal.title;
    if (proposal.author)
        dataToUpdate.author = proposal.author;
    if (proposal.synopsis)
        dataToUpdate.synopsis = proposal.synopsis;
    if (proposal.coverUrl)
        dataToUpdate.coverUrl = proposal.coverUrl;
    ...
    await db_1.default.$transaction([
        ...
        db_1.default.editProposal.update({
            where: { id: proposalId },
            data: { status: 'APPROVED' }
        }),
...
exports.rejectProposal = (0, express_async_handler_1.default)(async (req, res) => {
    const { proposalId } = req.params;
    const { reason } = req.body;
    // 1. Actualizamos el estado de la propuesta a RECHAZADA.
    // Guardamos el resultado en una variable para obtener los detalles.
    const rejectedProposal = await db_1.default.editProposal.update({
        where: { id: proposalId },
        data: { status: 'REJECTED', rejectionReason: reason },
...
exports.lockMangaForReview = (0, express_async_handler_1.default)(async (req, res) => {
    const { mangaId } = req.params;
...
exports.unlockManga = (0, express_async_handler_1.default)(async (req, res) => {
    const { mangaId } = req.params;
    const adminId = req.user.id;
    ...
    await db_1.default.manga.updateMany({
        where: {
            id: mangaId,
            lockedById: adminId,
        },
...
exports.getArchivedMangas = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    // 2. Definimos la condición de búsqueda usando el enum para seguridad de tipos
    const whereCondition = {
        status: client_1.ContentStatus.ARCHIVED
    };
    // 3. Ejecutamos la consulta paginada y el conteo total en una transacción
    const [archived, total] = await db_1.default.$transaction([
        db_1.default.manga.findMany({
            where: whereCondition,
            select: {
                id: true,
...
exports.deleteChapterPermanently = (0, express_async_handler_1.default)(async (req, res) => {
    const { chapterId } = req.params;
    // Usamos delete. Si el capítulo no existe, Prisma lanzará un error que asyncHandler capturará.
    await db_1.default.chapter.delete({
        where: { id: chapterId },
    });
...
exports.restoreManga = (0, express_async_handler_1.default)(async (req, res) => {
    const { mangaId } = req.params;
    // Buscamos el manga para asegurarnos de que realmente está archivado
    const mangaToRestore = await db_1.default.manga.findFirst({
