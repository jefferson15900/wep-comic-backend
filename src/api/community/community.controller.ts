...
export const createManga = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const coverUrl = (req.file as Express.Multer.File)?.path; 
  ...
  const rating = Object.values(ContentRating).includes(contentRating) ? contentRating : ContentRating.SFW;
...
export const addChapterToManga = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  const pages = req.files as Express.Multer.File[];
...
export const editMangaMetadata = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  const { mangaId } = req.params;
  ...
  const newCoverUrl = (req.file as Express.Multer.File)?.path;
...
export const editChapterMetadata = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
...
export const replaceChapterPages = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  const { chapterId } = req.params;
  ...
  const deleteOldPages = prisma.page.deleteMany({
    where: { chapterId: chapterId },
  });

  const updateChapterWithNewPages = prisma.chapter.update({
    where: { id: chapterId },
    data: {
      status: 'PENDING',
      lastEditedById: userId,
...
export const proposeMangaEdit = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { mangaId } = req.params;
  ...
  const existingPendingProposal = await prisma.editProposal.findFirst({
    where: {
      mangaId: mangaId,
  ...
  const newProposal = await prisma.editProposal.create({
    data: {
      mangaId,
      proposerId,
      justification,
...
export const archiveManga = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { mangaId } = req.params;
  const userId = req.user!.id;
